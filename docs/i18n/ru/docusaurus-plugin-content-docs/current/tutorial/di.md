...
В нашем приложении уже есть сущности пользователя и заказов, давайте представим что при покупке пользователю начисляются бонусы, и мы бы хотели после покупки обновить для пользователя информацию о его текущем количестве бонусов.
...
Кроме сервиса можно инжектить любой класс отнаследовав его от базового класса [Dependency](../api/services/dependency). 

Давайте рассмотрим пример, когда это может быть полезно. В нашем приложении нам необходимо делать запросы к бекенду, сейчас это описано набором утилит `fetchUser`, `fetchOrders` и т.д. Такой подход не предоставляет никакого явного контракта, работу с бекендом сложнее тестировать и настраивать, давайте это исправим.
...
Теперь у нашего АПИ есть явный контракт, а наши сервисы гораздо проще тестировать, тк мы можем подставлять любую реализацию контракта для АПИ.
