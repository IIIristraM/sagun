...
Ранее мы рассмотрели как читать данные, но нам еще нужно как-то реагировать на действия пользователя, чтобы изменять данные.

## Декоратор daemon

Давайте создадим еще одну сущность - Заказ. И кроме того, что мы можем загрузить заказы, мы еще можем создать новый заказ.
...
Дефолтное поведение декоратора `daemon` - метод не будет вызван пока не отработал текущий вызов (касается только вызова через actions). Это позволяет из коробки избегать избыточные срабатывания евентов - лишние клики по кнопкам, множественное событие скролла при пагинации и тд. 
...
Можно заметить, что каждый раз, когда мы добавляем заказ то видим лоадер, это не очень приятный UX, к тому же мы делаем лишний запросы за списком заказов.
...
// - по умолчанию, каждый раз когда выполняется асинхронная операция ее прошлый результат обнуляется,
...
разного рода мердж данных и т.д.
